...
parser.add_argument(
    "--batch_size", default="100", type=int, help="Batch size", dest="batch_size"
)
parser.add_argument(
    "--mil_pooling_filter",
...
parser.add_argument(
    ...
    help="Text file to write test metrics",
...
parser.add_argument(
    ...
    default="100",
    type=int,
    help="number of bags per image to be tested",
    dest="num_bags_per_image",
)
...
model_name = FLAGS.init_model_file
# if len(model_name) == 0:
#     model_name =
# model_name = (
#     FLAGS.init_model_file.split("__")[1]
#     + "__"
#     + FLAGS.init_model_file.split("__")[2]
#     + "__"
#     + FLAGS.init_model_file.split("__")[3][:-4]
# )
data_folder_path = "{}/{}/{}".format(
    FLAGS.test_metrics_dir,
    model_name.split("/")[1].removesuffix(".pth"),
    FLAGS.dataset_type,
)
# breakpoint()
...
model = Model(
    ...
    sigma=FLAGS.sigma,
    M=state_dict['config']["M"],
    K=state_dict['config']["K"],
    T=state_dict['config']["T"],
    no_sigmoid=state_dict['config']["no_sigmoid"]
)
model.to(device)
...
with torch.no_grad():
    ...
    for i in range(num_images):
        ...
        test_metrics_filename = "{}/bag_predictions_{}.txt".format(
            image_data_folder_path, image_id
        )
